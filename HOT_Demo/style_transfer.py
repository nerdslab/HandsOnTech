...
def tensor_load_rgbimage2(img, size=None, scale=None, keep_asp=False):
    img = img-np.min(img)
...
def magic_box(content_image, style_image, out_name, workingdir,cff,sff):
    ...
    for i in [3, 4]:
        for j in [0, 3, 6]:
            state_dict.pop('model1.' + str(i) + '.conv_block.' + str(j) + '.running_mean', 'None')
            state_dict.pop('model1.' + str(i) + '.conv_block.' + str(j) + '.running_var', 'None')
    state_dict.pop('model.0.1.running_mean', 'None')
    state_dict.pop('model.0.1.running_var', 'None')
    for i in [3, 4]:
        for j in [0, 3, 6]:
            state_dict.pop('model.0.' + str(i) + '.conv_block.' + str(j) + '.running_mean', 'None')
            state_dict.pop('model.0.' + str(i) + '.conv_block.' + str(j) + '.running_var', 'None')
    for n in [2, 3, 4, 5, 6, 7, 8, 9]:
        state_dict.pop('model.' + str(n) + '.1.running_mean', 'None')
        state_dict.pop('model.' + str(n) + '.1.running_var', 'None')
        for i in [3, 4]:
            for j in [0, 3, 6]:
                state_dict.pop('model.' + str(n) + '.conv_block.' + str(j) + '.running_mean', 'None')
                state_dict.pop('model.' + str(n) + '.conv_block.' + str(j) + '.running_var', 'None')
    state_dict.pop('model.10.running_mean', 'None')
    state_dict.pop('model.10.running_var', 'None')

    style_model = Net(ngf=128)
    style_model.load_state_dict(state_dict, False)

    style_v = Variable(style)
    content_image = Variable(preprocess_batch(content_image))
    style_model.setTarget(style_v)
    output = style_model(content_image)
    tensor_save_bgrimage(output.data[0], out_name, False)
    return imread(out_name)
